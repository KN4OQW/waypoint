...
func TestFetchAtomicAndFailureSafe(t *testing.T) {
	...
	defer srv.Close()
	if err := Fetch(context.Background(), []string{srv.URL}, path, verifydl.Verify{}); err != nil {
		t.Fatal(err)
	}
	...
	defer bad.Close()
	if err := Fetch(context.Background(), []string{bad.URL}, path, verifydl.Verify{}); err == nil {
