...
| DMR/CCS7 ID *(WPSD; Pi-Star "CCS7/DMR ID")* | `[General] Id`, `[DMR] Id` | `general.id` | done | `dmr.id` overrides for DMR; falls back to `general.id`. The two fields are labelled to say so — "DMR ID" on General, "DMR ID override" on the DMR panel with the inherited value in its placeholder (#140). The General Callsign field is a type-ahead over the cached `DMRIds.dat`, and choosing a row fills the ID in. |
