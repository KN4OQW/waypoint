...
- **D2 — waypoint-stack, follow-up.** No `waypoint-bus@.service` template unit exists on the box. One was created ad-hoc for this validation; it must be added to **waypoint-stack**, and the render/apply path must also *enable* the instance for boot persistence (nothing does today; the instance was enabled by hand).
  - **Resolved, both halves — but not where this said to put it.** Apply enables and disables bus instances for boot via `BootEnableUnits`/`BootDisableUnits`, which is what the reboot run below exercises. The template did land in waypoint-stack, and that turned out to be the wrong home: the stack `.debs` ship no unit files, so nothing installed it, and the daemon it names was never built by CI or published to a release either — the bus UI shipped to operators with no daemon behind it for a full release. Unit and binary both moved to the image, and the binary became a signed release asset (issue #109). What misled this entry is that waypoint-stack is where the *daemons* live; it is not where anything that installs a unit lives.
