...
- **Phonebook**: identity and contact detail for the operators a node knows — callsign, optional DMR ID, name, email — in its own schema-versioned table beside the settings key tree, not in it. It compiles to no INI key, so it is outside the render/apply spine entirely: adding an entry restarts nothing. Reached only through the authenticated `/api/phonebook` surface; the email column is PII and appears in no public-view response type and in no log. The surrogate `id` is the anchor rather than the callsign, so later units can reference a person by foreign key without depending on a licence that can be reassigned or a DMR ID issued per radio.

### Supervisor
- Owns systemd units for stack daemons; derives desired-state from the config store.
- **One desired-state set, used three ways.** `RenderTargets` answers "what should run on this node" — the mode is enabled, no bus has displaced its gateway, and every value the daemon needs at startup is set (`UnmetGatewayRequirements`). Apply writes exactly those configs, restarts exactly those units, and enables exactly those units for boot; everything else in the managed set is stopped and boot-disabled. The boot picture is *derived* from the render set rather than listed alongside it, because the two lists drifted when they were separate: `waypoint-mmdvm.service` appeared in neither, so the modem host died at every reboot and returned only when someone applied config — which also meant every stack update on a rebooted node failed its health gate — while the YSF and NXDN gateways were boot-enabled unconditionally, starting daemons whose modes were switched off. A mode added later is covered the day it gets a render target.
- **Startup requirements** are the "and the configuration is correct" half of that predicate, and they are deliberately narrow: a daemon is withheld only for something knowable locally. DAPNETGateway exits without a DAPNET AuthKey and MMDVM-Host cannot open a port that was never set, so both are gated; whether the modem hardware actually answers is a *runtime* condition and is not. The one non-crash gate is identity — MMDVM-Host starts happily with no callsign or ID and would transmit unidentified, which a station may not do. No requirement is invented for a daemon that would otherwise run.
