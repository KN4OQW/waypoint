...
- Calibration ([RFC-0021](rfcs/0021-guided-calibration.md)): a guided RX/TX offset sweep with a live BER readout, measured by the node itself against the operator's radio. **Native Go, not MMDVMCal** — the modem computes no BER, so a subprocess would buy one function and charge a pty, a screen-scrape and an image dependency for it; the FEC arithmetic is ported and none of its tables are. Repeater level/invert workflow included, unverified on hardware.
